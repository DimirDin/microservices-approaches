...
&gt; **Контекст:** Вы работаете в крупной компании, которая строит систему на основе микросервисной архитектуры. Вам как DevOps-специалисту необходимо выдвинуть предложение по организации инфраструктуры для разработки и эксплуатации.
...
- **GitLab SaaS** хранит репозитории каждого микросервиса и `.gitlab-ci.yml` pipelines
- **Central Templates Repo** — центральный репозиторий с шаблонами CI/CD, подключаемыми через `include` в каждый сервис
- **GitLab Runners** (Kubernetes Cluster) — агенты сборки на собственных серверах с Docker/Kubernetes executor, autoscaling от 0 до N подов
- **Custom Docker Images** — собственные образы для сборки и тестирования, хранятся в GitLab Container Registry
- **Secrets Management** — HashiCorp Vault или GitLab CI/CD Variables для безопасного хранения паролей и ключей
...
- **Vector DaemonSet** — 1 агент на каждую ноду Kubernetes, собирает stdout/stderr из `/var/log/pods`, обогащает метаданными (pod labels, namespace, node name), буферизует через WAL на локальном диске
- **Loki Stack** — Distributor (приём логов), Ingester (WAL + индексация labels), Querier (поиск по labels + chunk storage), хранит chunk-и в Object Storage (S3/MinIO/GCS)
- **Grafana** — единый UI для Explore (ad-hoc поиск через LogQL), Dashboards (логи + метрики), RBAC (организации, команды, view-only доступ)
...
- **Prometheus Server** — скрейпит метрики каждые 15 секунд, Service Discovery через Kubernetes API, relabeling для добавления service labels/namespace/pod, локальное хранение 15 дней, long-term → Thanos
- **Node Exporter** — на каждом хосте, собирает CPU, RAM, диск, сеть (100+ метрик)
- **cAdvisor** — на каждом хосте, container-level метрики (CPU, RAM, диск, сеть по контейнерам)
- **kube-state-metrics** — метрики Kubernetes объектов (pods, deployments, replicasets)
- **Grafana** — Dashboards (templating variables: $service, $namespace, $node), Explore (ad-hoc PromQL), Alertmanager интеграция
- **Alertmanager** → Slack / PagerDuty / Webhook (grouping, inhibition, routing)
